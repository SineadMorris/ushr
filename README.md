...
Note that to install the package with its vignette, you must first install `knitr`, then install `ushr` with `build_vignettes = TRUE`. The package vignette can then be viewed using `browseVignettes()`.
